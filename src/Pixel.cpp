...
void Pixel::setId(int id) { m_id = id; }
...
int Pixel::getId() { return m_id; }

float Pixel::getRedColor() { return m_redColor; }

float Pixel::getGreenColor() { return m_greenColor; }

float Pixel::getBlueColor() { return m_blueColor; }

void Pixel::sendData(Pixel *pPixels, SerialPort *pArduino,
                     const int &NUM_LEDS) {
    ...
    do {
        // printf("0x%02x\n", ack);
        while (!pArduino->writeSerialPort(&HEADER_BYTE, 1))
            ;
        usleep(10 * 1000);

    } while (pArduino->readSerialPort(&ack, 1) <= 0 || ack != '1');

    while (!pArduino->writeSerialPort(&CMD_FRAME_DATA, 1))
        ;
    ack = '0';
    ...
    for (size_t i = 0; i < TOTAL_SIZE; i += CHUNK_SIZE) {
        ...
        do {
            ...
            while (!pArduino->writeSerialPort(frameData + i + offset,
                                              sizeToSend - offset))
                ;
...
void Pixel::setLerpSpeed(SerialPort *pArduino, int8_t lerpSpeed) {
    if (!pArduino->isConnected()) {
        return;
    }
    ...
    pArduino->writeSerialPort(&CMD_SET_SETTINGS, 1);
    pArduino->writeSerialPort(&SET_ID_LERP, 1);
    pArduino->writeSerialPort((char *)&lerpSpeed, 1);
